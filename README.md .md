...
In this report , we will go through the various steps of creating **searching engine** and more .
...
```
> After creating the interface its time to add the tools we discussed before.
...
```
>  Now we can find any cell only by typing the correct coordinates otherwise it wont accept it.
...
```
>Now we get to locate the data inside our spreadsheet .
# Conclusion 

Making use of both **UI Widgets** and **Brute Coding** , enabled us to create a spreadsheet from scratch  ,  it's been a great working on this project .
